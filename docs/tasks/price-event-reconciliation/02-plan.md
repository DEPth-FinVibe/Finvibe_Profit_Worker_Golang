...
| D4 최신성 판정 규칙 | 사용자 확인 필요 | 선택 후 S4 구현 |

세부 근거와 트레이드오프는 `03-decisions.md`에서 관리한다.

## D4 선택지: 최신성 판정 규칙

1. `updatedAt`을 우선 비교하고, 같은 시각의 가격 충돌에서는 먼저 확정된 값을 유지한다.
   - 장점: DLT·재처리 과거 이벤트의 롤백과 동일 시각 값의 반복 변경을 막는다.
   - 단점: 같은 시각에 수정된 authoritative 가격을 Go 이벤트만으로 교정할 수 없다.
2. 가격이 다르면 수신 순서대로 항상 적용한다.
   - 장점: 구현이 단순하다.
   - 단점: 오래된 DLT 이벤트가 최신 평가 상태를 되돌릴 수 있다.
3. Kafka partition과 offset만 비교한다.
   - 장점: 같은 partition 안의 순서를 명확히 판단한다.
   - 단점: DLT 이동 뒤 원본 offset 계약과 Batch 가격 상태를 함께 비교하기 어렵다.

AI 추천: **1번**. 기본 topic, DLT와 Batch가 공통으로 사용할 수 있는 기준은 `updatedAt`이며, 동일 시각 충돌은 안정적인 단일 결과를 유지하는 편이 안전하다.
